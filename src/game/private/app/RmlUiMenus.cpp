...
bool RmlUiMenus::Init(RmlUiHost &host, const std::string &dataDir)
{
    ...
    if (menuDoc_ == nullptr || setupDoc_ == nullptr || settingsDoc_ == nullptr ||
        loadDoc_ == nullptr || confirmDoc_ == nullptr)
    {
        Shutdown();
        return false;
    }
...
void RmlUiMenus::Shutdown()
{
    if (context_ != nullptr)
    {
        for (Rml::ElementDocument *doc :
             { menuDoc_, setupDoc_, settingsDoc_, loadDoc_, confirmDoc_ })
        {
            if (doc != nullptr)
            {
    ...
    menuDoc_ = setupDoc_ = settingsDoc_ = loadDoc_ = confirmDoc_ = nullptr;
    context_ = nullptr;
    host_ = nullptr;
    ready_ = false;
...
void RmlUiMenus::OnClick(const Rml::String &id)
{
    ...
    {
        // BeginRemap announces internally and moves to HotkeyRemap, which
        // stays with the raygui branch until Phase 4.
        screens_.BeginRemap(MenuState::Settings);
    }
    else if (id == "btn-yes")
    {
