...
void RunAttackGroundTests()
{
    // --- out-of-range order marches, then holds and shells ---
    {
        ...
        Orders &gunOrders = GetOrders(registry, id);

        IssueAttackGroundOrder(gun, gunOrders, GetMover(registry, id), map,
                               cc::ToRaylib(cc::TileToWorld(15, 2)));
        CC_CHECK(gunOrders.hasAttackGroundOrder);
        ...
        CC_CHECK(GetOrders(registry, id).hasAttackGroundOrder); // standing order, not one-shot
    }
    ...
    // --- enemy at the impact point takes damage through the phase machine ---
    {
        ...
        IssueAttackGroundOrder(*registry.Get<Unit>(id), GetOrders(registry, id),
                               GetMover(registry, id), map,
                               cc::ToRaylib(cc::TileToWorld(15, 2)));
    ...
    // --- empty ground: no crash, order persists (continuous, not one-shot) ---
    {
        ...
        IssueAttackGroundOrder(gun, GetOrders(registry, id), GetMover(registry, id), map,
                               cc::ToRaylib(cc::TileToWorld(10, 2)));
        for (int i = 0; i < 30; ++i)
        {
            UpdateUnit(id, registry, map, 1.0f); // must not crash, hits nothing
        }
        CC_CHECK(GetOrders(registry, id).hasAttackGroundOrder);
    }
    ...
    // --- ResolveGroundAttack: nearest in splash takes it, corpses excluded ---
    {
        ...
        ResolveGroundAttack(registry, gun, GetCombatState(registry, id),
                            cc::ToRaylib(cc::TileToWorld(5, 5)));
    ...
    // --- new move orders clear shelling (exactly-one-active invariant) ---
    {
        ...
        CC_CHECK(GetOrders(registry, id).hasAttackGroundOrder);
        IssueMoveOrder(gun, GetOrders(registry, id), GetMover(registry, id),
                       cc::ToRaylib(cc::TileToWorld(1, 8)));
        CC_CHECK(!GetOrders(registry, id).hasAttackGroundOrder);
    }
    ...
    // --- G2 arc: WindUp launches a shell, impact lands on the ordered tile ---
    {
        ...
        const Entity id = SpawnArty(registry, 2, 2);
        Unit &gun = *registry.Get<Unit>(id);
        CombatState &gunCombat = GetCombatState(registry, id);
        const Vector2 impact = cc::ToRaylib(cc::TileToWorld(6, 2));
        ResolveStrikeGround(registry, id, gun, gunCombat, impact);
        int shells = 0;
        Vector2 landing = {};
        ...
        CC_CHECK(gunCombat.cooldown == gun.cooldownTime); // cooldown stamped at launch
    ...
    // --- G2 AoE: close enemy hit, distant enemy and friendlies spared ---
    {
        ...
        ResolveStrikeGround(registry, id, gun, GetCombatState(registry, id), impact);
    ...
    // --- G2 dodge: a fast vehicle escapes the snapshot landing point ---
    {
        ...
        ResolveStrikeGround(registry, id, gun, GetCombatState(registry, id),
                            { runner.position.x + 32.0f, runner.position.y + 32.0f });
        // Plain move orders never acquire, so the runner marches east flat-out.
        IssueMoveOrder(*registry.Get<Unit>(foe), GetOrders(registry, foe),
                       GetMover(registry, foe),
